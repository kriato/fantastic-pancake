  ...
  let disposable = vscode.commands.registerCommand('fantastic-pancake.run', () => {
    ...
    vscode.window.showInputBox({ prompt: 'Method to use', placeHolder: 'Insert desired method here' }).then(value => {
      ...
      if (activeEditor) {
        ...
        let nextLineContent = activeEditor.document.lineAt(cursorRow + i).text;
        ...
        activeEditor.edit(editBuilder => {
          ...
          if (value === PRINT_LABEL) {
            if (i === 0) {
              editBuilder.insert(new vscode.Position(cursorRow + 1, nextLineCursor), '\n' + nextLineOffset + desiredCommandStart + word + desiredCommandEnd);
            }
            else {
